...
- 错误类型丢失信息（map\_err 剥成 String）：保留错误链；库用 thiserror 枚举。
- ? 的 From 转换没实现：编译错误指引你加 #\[from] 或手动 map\_err。
...
- catch\_unwind 试图「捕获一切继续跑」：panic 后的不变量破坏不可信；恢复要谨慎（[第 4 篇](../c/12-errors-robustness.md)的事务边界）。

## 7. 小结

- 失败二分法：panic 管 bug（不可恢复、尽早暴露）、Result 管现实失败（可恢复、调用方决策）——判据是「bug 还是环境的现实」。
- ? 是错误传播的语法糖：Ok 解包、Err 上抛、From 自动跨类型转换——「洋葱式错误链」的语言级机制。
- 组合子把错误处理变成值的管道：unwrap\_or/and\_then/map\_err 与 ? 的分工（默认/链式 vs 早退）。
...
> [!TIP]
> 思路简单线性流用 ?（主路径清晰）、需要「失败降级/多分支」用组合子。混合使用是常态：? 主干 + 局部 unwrap\_or。
